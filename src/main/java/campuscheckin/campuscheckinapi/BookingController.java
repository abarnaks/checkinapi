...
public class BookingController {
	...
	public List<Booking> getBookingsByUserId(@PathVariable("id") String userId){
		...
		try {
			...
			long endTime = System.currentTimeMillis();
			logRepo.save(new Log("/booking/findBookings/{id}", startTime,endTime, "S" ));
		} catch (Exception e) {
			long endTime = System.currentTimeMillis();
			logRepo.save(new Log("/booking/findBookings/{id}", startTime,endTime, "F" ));
			bookings = null;
		}
	...
	public List<Booking> getBookingsByRoomId(@PathVariable("id") String roomId){
		...
		try {
			...
			long endTime = System.currentTimeMillis();
			logRepo.save(new Log("/booking/roombooking/{id}", startTime,endTime, "S" ));
		} catch (Exception e) {
			long endTime = System.currentTimeMillis();
			logRepo.save(new Log("/booking/roombooking/{id}", startTime,endTime, "F" ));
			bookings = null;
		}
		return bookings;
	}
